...
void drawBlobInfoOnImage(vector<Blob> &blobs, Mat &imgFrame2Copy) {
	...
	for (unsigned int i = 0; i < blobs.size(); i++) {
		...
		if (blobs[i].StillBeingTracked == true) {
			...
			double FontScale = blobs[i].CurrentDiagonalSize / 60.0;
			int FontThickness = (int)round(FontScale * 1.0);

			putText(imgFrame2Copy, to_string(i), blobs[i].centerPositions.back(), FontFace, FontScale, SCALAR_GREEN, FontThickness);
		}
	}
}
